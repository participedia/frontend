// Copied from react-s3-uploader, but with none of the extraneous dependencies
...
function S3Upload(options) {
  ...
  let files = this.fileElement ? this.fileElement.files : this.files || [];
...
S3Upload.prototype.executeOnSignedUrl = function(file, callback) {
  ...
  if (this.signingUrlQueryParams) {
    ...
    Object.keys(signingUrlQueryParams).forEach(function(key) {
      let val = signingUrlQueryParams[key];
      queryString += "&" + key + "=" + val;
    });
  }
...
S3Upload.prototype.uploadToS3 = function(file, signResult) {
  let xhr = this.createCORSRequest("PUT", signResult.signedUrl);
  if (!xhr) {
  ...
  if (this.uploadRequestHeaders) {
    let uploadRequestHeaders = this.uploadRequestHeaders;
    Object.keys(uploadRequestHeaders).forEach(function(key) {
      let val = uploadRequestHeaders[key];
      xhr.setRequestHeader(key, val);
    });
  } else {
